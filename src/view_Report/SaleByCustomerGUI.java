package view_Report;

import javax.swing.JPanel;
import javax.swing.JScrollPane;
import javax.swing.JTable;
import javax.swing.UIManager;
import javax.swing.UIManager.LookAndFeelInfo;
import javax.swing.table.DefaultTableModel;

import model_MenuSection.DbLink;

import java.awt.BorderLayout;
import java.awt.Color;
import java.text.SimpleDateFormat;

import javax.swing.BorderFactory;
import javax.swing.JLabel;

import controller_Report.SaleByCustomer_Handler;

import java.awt.Font;
import java.awt.SystemColor;
import java.sql.ResultSet;
import java.sql.SQLException;

import org.jdesktop.swingx.JXDatePicker;

public class SaleByCustomerGUI {

//	private JFrame frame;
//
//	/**
//	 * Launch the application.
//	 */
//	public static void main(String[] args) {
//		EventQueue.invokeLater(new Runnable() {
//			public void run() {
//				try {
//					SaleReportGUI window = new SaleReportGUI();
//					window.frame.setVisible(true);
//				} catch (Exception e) {
//					e.printStackTrace();
//				}
//			}
//		});
//	}

	/**
	 * Create the application.
	 */
	public SaleByCustomerGUI() {

		DbLink.connect();
		initialize();
	}

	/**
	 * Initialize the contents of the frame.
	 */
	public JPanel panel ;
	public JLabel totalNetSale,refundLabel;
	public DefaultTableModel model,customerModel;
	public JTable table,ctable;
	public JXDatePicker dField;
	SaleByCustomer_Handler handler = null;
	public JLabel discountLabel;
	private void initialize() {
		handler= new SaleByCustomer_Handler(this);
		//frame = new JFrame();
		panel = new JPanel();
	    add();
	}
	
	@SuppressWarnings("serial")
	void add(){
		try {
		    for (LookAndFeelInfo info : UIManager.getInstalledLookAndFeels()) {
		        if ("Nimbus".equals(info.getName())) {
		            UIManager.setLookAndFeel(info.getClassName());
		            break;
		        }
		    }
		} catch (Exception e) {
			System.out.print("Nimbus not Found");
		}
//		frame.setBounds(100, 100, 887, 623);
//		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
//		frame.getContentPane().setLayout(null);
		SimpleDateFormat simpleDateFormat = new SimpleDateFormat("dd/MM/yyyy");
		panel.setBackground(Color.WHITE);
		panel.setBounds(51, 36, 793, 536);
		//frame.getContentPane().add(panel);
		panel.setLayout(null);
		
		String[] columnTitle = {"Line","Invoice Code","Net Total","Discount","Refund"};
		model = new DefaultTableModel(columnTitle, 0)
		{
			public boolean isCellEditable(int row, int column) {
			    return (column==8);  
				}
			};
		JPanel tablePanel = new JPanel();
		tablePanel.setFocusable(false);
		tablePanel.setBounds(3, 100, 389, 436);
		tablePanel.setLayout(new BorderLayout());
		table  = new JTable(model);
		table.setRowHeight(30);
		table.getColumnModel().getColumn(0).setMaxWidth(36);
		table.getColumnModel().getColumn(1).setMinWidth(90);
		table.setFont(new Font("Dialog", Font.PLAIN, 14));
		table.setEnabled(false);
		table.setRowSelectionAllowed(false);
		table.setFocusable(false);
		table.setFillsViewportHeight(true);
		table.setLocation(21, 184);
		table.setSize(629, 500);
		tablePanel.add(table.getTableHeader(),BorderLayout.NORTH);
		JScrollPane scrollPane = new JScrollPane(table);
		scrollPane.setFocusable(false);
		tablePanel.add(scrollPane,BorderLayout.CENTER);
		panel.add(tablePanel);
	    
	    JPanel panel_1 = new JPanel();
	    panel_1.setBounds(0, 0, 789, 101);
	    panel_1.setLayout(null);
	    panel_1.setBorder(BorderFactory.createTitledBorder("Filter by date"));
	    panel.add(panel_1);
	    
	    totalNetSale = new JLabel();
	    totalNetSale.setBounds(544, 18, 233, 30);
	    panel_1.add(totalNetSale);
	    totalNetSale.setText("Purchase: ");
	    totalNetSale.setFont(new Font("Dialog", Font.PLAIN, 15));
	    totalNetSale.setForeground(Color.DARK_GRAY);
	    totalNetSale.setFocusable(false);
	    dField = new JXDatePicker();
	    dField.setToolTipText("Apply filter after customer \nselection from customer table");
	    dField.setBounds(22, 24, 148, 30);
	    panel_1.add(dField);
	    dField.setOpaque(false);
	    dField.addActionListener(handler);
	    dField.getEditor().setFont(new Font("Tahoma", Font.PLAIN, 14));
	    dField.setFormats(simpleDateFormat);
	    dField.setFont(new Font("Tahoma", Font.PLAIN, 12));
	    
	    refundLabel = new JLabel();
	    refundLabel.setText("Refund: ");
	    refundLabel.setForeground(Color.DARK_GRAY);
	    refundLabel.setFont(new Font("Dialog", Font.PLAIN, 15));
	    refundLabel.setFocusable(false);
	    refundLabel.setBounds(544, 68, 233, 30);
	    panel_1.add(refundLabel);
	    
	    discountLabel = new JLabel();
	    discountLabel.setText("Discount: ");
	    discountLabel.setForeground(Color.DARK_GRAY);
	    discountLabel.setFont(new Font("Dialog", Font.PLAIN, 15));
	    discountLabel.setFocusable(false);
	    discountLabel.setBounds(544, 42, 233, 30);
	    panel_1.add(discountLabel);
	    panel.setVisible(true);
	    
	    
	    String[] customerColumnTitle = {"Name","Contact ID","Address"};
		customerModel = new DefaultTableModel(customerColumnTitle, 0){
		public boolean isCellEditable(int row, int column) {
		       switch(column){
		       default:return false;
		       }
		   }
		};
				
		JPanel customerPanel = new JPanel();
		customerPanel.setBounds(391, 100, 398, 436);
		customerPanel.setBorder(UIManager.getBorder("DesktopIcon.border"));
		customerPanel.setBorder(BorderFactory.createTitledBorder("Customers"));
		customerPanel.setLayout(new BorderLayout());
		customerPanel.setBackground(new Color(230, 230, 230));
		ctable  = new JTable(customerModel);
		ctable.getColumnModel().getColumn(0).setMaxWidth(115);
		ctable.getColumnModel().getColumn(1).setMaxWidth(105);
		ctable.getColumnModel().getColumn(1).setMinWidth(103);
		ctable.setRowHeight(30);
		ctable.setFont(new Font("Dialog", Font.PLAIN, 14));
		ctable.setBackground(SystemColor.windowBorder);
		ctable.setBorder(null);
		ctable.setFillsViewportHeight(true);
		ctable.addMouseListener(handler);
		ctable.putClientProperty("terminateEditOnFocusLost", Boolean.TRUE);
		table.setLocation(21, 184);
		ctable.setFocusable(false);
		ctable.setSize(629, 500);
		ctable.setLocation(21, 184);
		ctable.setSize(629, 500);
		customerPanel.add(ctable.getTableHeader(),BorderLayout.NORTH);
		customerPanel.add(new JScrollPane(ctable),BorderLayout.CENTER);
		panel.add(customerPanel);
		addData();
		
	}
	void addData(){
			try{ResultSet rs = null;
			rs  = DbLink.query("Select * from geranium.customers");
				while(rs.next()){
					Object[] obj = {rs.getString("Name"),rs.getString("CUSTOMER_ID"),rs.getString("Address")};
					customerModel.addRow(obj);
				}
			} catch (SQLException e) {
			e.printStackTrace();
		}
	}
}
