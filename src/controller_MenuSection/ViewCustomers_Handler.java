...
import view_MenuSection.ViewCustomers_GUI;


public class ViewCustomers_Handler implements ActionListener,KeyListener{

	ViewCustomers_GUI gui;
	int fileIndex = 1;
	public ViewCustomers_Handler(ViewCustomers_GUI viewCustomer) {
		gui = viewCustomer;
		DbLink.connect();
	}
	...
	void searCustomer(String code) {
		if(!(code.equals("") && code.equals(null))){
			try {
				gui.model.setRowCount(0);
				int i = 0;
				ResultSet rs = DbLink.query("SELECT * FROM GERANIUM.CUSTOMERS WHERE Customer_ID LIKE '"+code+"%' ORDER BY CUSTOMER_ID");
				while(rs.next()){
					i++;
					Object[] obj = {i,rs.getString("CUSTOMER_ID"),rs.getString("NAME"),rs.getString("ADDRESS")};
	...
	void updateTablede(){
		...
		try {
			...
			str.add(gui.table.getValueAt(row, 3));
					...
					if(DbLink.updateExistedCustomers(str.get(0).toString(),str.get(1).toString().trim(), str.get(2).toString())){
							gui.table.setSelectionBackground(Color.blue);
					}
					else{
			...
			Toolkit.getDefaultToolkit().beep();
			JOptionPane.showMessageDialog(null, "Select Customer fist!");
		}
	}
	...
	public void keyReleased(KeyEvent e) {
		gui.table.setSelectionBackground(Color.blue);
		searCustomer(gui.idField.getText().trim());
	}
